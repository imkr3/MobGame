...
class World(private val fx: Fx, private val haptics: Haptics) {
    ...
    val player = Player()
    private val bullets = Array(620) { Bullet() }
    private var bIdx = 0
    private val enemies = Array(56) { Enemy() }
    ...
    private fun updateWaveDirector(dt: Float) {
        ...
        if (scriptIdx >= script.size && activeEnemies() == 0 && !gameOver) {
            val bonus = 400 * wave + player.lives * 150
            score += bonus
    ...
    private fun updatePlayer(dt: Float) {
        ...
        val px = player.x
        player.x = approach(player.x, player.tx, 0.42f, dt)
        player.y = approach(player.y, player.ty, 0.42f, dt)
    ...
    private fun playerShot(offX: Float, offY: Float, angleDeg: Float, r: Float, dmg: Int) {
        val od = player.odTime > 0f
        val a = (-90f + angleDeg) * DEG
        val speed = 1000f
        val color = if (od) Palette.AMBER else Palette.CYAN
        fireAngle(player.x + offX, player.y + offY, a, speed, r, dmg, false, color, 1)
    }

    private fun playerFire() {
        val od = player.odTime > 0f
        val d = if (od) 4 else 2
        when (player.weapon) {
        ...
        for (b in bullets) {
            ...
            for (e in enemies) {
                ...
                if (dx * dx + dy * dy <= rr * rr) {
                    b.active = false
                    damageEnemy(e, b.damage.toFloat(), b.x, b.y)
                    break
                }
            }
        }
        ...
        if (player.alive) {
            ...
            for (b in bullets) {
                ...
                if (d2 <= hitR * hitR) {
                    ...
                    b.grazed = true
                    player.overdrive = clamp(player.overdrive + 0.035f, 0f, 1f)
                    score += 15
            ...
            for (e in enemies) {
                ...
                if (dx * dx + dy * dy <= rr * rr) {
                    if (player.odTime > 0f) {
                        damageEnemy(e, 6f, player.x, player.y)
                    } else if (player.invuln <= 0f) {
                        if (e.kind != EK.BOSS) damageEnemy(e, 999f, e.x, e.y)
                        hurtPlayer()
                        return
                    }
                }
    ...
    private fun collect(u: PowerUp) {
        when (u.kind) {
            ...
            PK.SHIELD -> {
                player.shield = (player.shield + 1).coerceAtMost(2)
    ...
    private fun damageEnemy(e: Enemy, dmg: Float, hx: Float, hy: Float) {
        e.hp -= dmg
        e.hitFlash = 0.12f
        fx.cone(hx, hy, 3, -TAU * 0.25f + rnd(-0.6f, 0.6f), 0.9f, Palette.WHITE, 170f, 1.8f, 0.22f)
        if (e.hp > 0f) return
        killEnemy(e)
    }
    ...
    private fun killEnemy(e: Enemy) {
        ...
        val gained = (e.score * multiplier).toInt()
        score += gained
